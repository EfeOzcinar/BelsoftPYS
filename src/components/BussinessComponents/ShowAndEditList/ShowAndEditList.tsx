...
interface  ShowAndEditListProps {
...
function ShowAndEditList(props: ShowAndEditListProps) {
    const {themeColor} = useAppContext();
    const {isDialogVisible, onClose, onSave, selectedList, header} = props;
    const [checkedItems, setCheckedItems] = useState(selectedList && selectedList.map(() => false));

    useEffect(() => {
        const pageLoad = () => {
            const initialCheckedItems = selectedList.map((item: any) => item.isStudentNonAttended === true);
    ...
    const handleCheckBoxToggle = (index: any) => {
        const updateChkItem = [...checkedItems];
        updateChkItem[index] = !updateChkItem[index];
        setCheckedItems(updateChkItem);
    };

    const landSelectedInfoList = () => {
        let selectedStudentInfo = selectedList.filter((item: any, index: any) => checkedItems[index]);
        return selectedStudentInfo;
    };

    const onSaveClick = () => {

        const selectedStudentList = landSelectedInfoList();
        const studentListParicipate = selectedList.filter((element: any) => !selectedStudentList.includes(element));

        const studentIdList_ = [] as any;
        if (selectedStudentList && selectedStudentList.length > 0) {
          selectedStudentList.forEach((element: any) => {
            studentIdList_.push(element.id);
          });
        }

        const studentIdListParicipate_ = [] as any;
        studentListParicipate.forEach((item: any) => {
            studentIdListParicipate_.push(item.id);
        });

        const studentsAttendance = {
            studentIdList: studentIdList_,
            studentIdListParicipate: studentIdListParicipate_,
        };
...
export default ShowAndEditList;