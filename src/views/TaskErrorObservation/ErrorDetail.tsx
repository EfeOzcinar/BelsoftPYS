...
import ImageViewer from '../../components/BussinessComponents/ImageViewer';
...
function ErrorDetail() {
    ...
    const [errorDetailList, setErrorDetailList] = useState<any[]>([]);
    // const [errorDocumentList, setErrorDocumentList] = useState<any[]>([]);
    ...
    const [errorExplanation, setErrorExplanation] = useState<any>('<p></p>');
    const [errorData, setErrorData] = useState<any>({});
    const [errorInProcess, setErrorInProcess] = useState<boolean>(false);
    ...
    useEffect(() => {
        async function pageLoad() {
            ...
            try {
                ...
                if (userDataSession) userInfo = JSON.parse(userDataSession);

                const favouriteList_ = await blsCore.services.errorService.getFavoriteInfoByErrorOid({
                ...
                isErrorSendUserEqualToCurrentUserRef.current =
                    userInfo.TitleCode === ParametrConstants.TITLE_MANAGEMENT ||
                    userInfo.TitleCode === ParametrConstants.TITLE_TEAM_LEAD;

                hideErrorAssignmentRef.current = data.ClosedDate > 0 && data.CloseUserOid > 0;
                setSelectedUserInfo(userInfo);

                await checkTaskOrErrorInProcess(userInfo.Oid, data.Oid);
    ...
    const observeDocument = async (selectedRow: any) => {
        ...
        setLoading(true);
        try {
            const docInfo: any = await observeSelectedFile(selectedRow, 'Errors');
    ...
    const fillArea = async (data: any) => {
        setLoading(true);
        try {
            const errorInfo = await blsCore.services.errorService.getErrorInfo(data.Oid);
            ...
            setErrorData(dataObj);
            setErrorDetailList(errorInfo.ErrorDetailList);
            setImageList(errorInfo.ErrorDocumentList); // imageList tabloya ekleniyor
            setErrorExplanation(data.Explanation);
        } catch (e) {
            console.log('fillArea error', e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const removeRequest = (id: number) => {
        const filteredRequests = errorDetailList.filter((request) => request.Oid !== id);
        setErrorDetailList(filteredRequests);
    };
    ...
    const processTask = async () => {
        ...
        try {
            ...
            if (errorInProcess) {
                ...
                setErrorInProcess(false);
                Toast.show({ type: 'success', text1: 'Hatanız durduruldu.' });
            }
            else {
                ...
                setErrorInProcess(true);
                Toast.show({ type: 'success', text1: 'Hatanız işleme alındı.' });
            }
        } catch (error) {
    ...
    const checkTaskOrErrorInProcess = async (userOid: number, taskOid: number) => {
        setLoading(true);
        try {
            const result = await blsCore.services.taskAndErrorService.checkTaskOrErrorInProcess({
                TaskOrErrorOid: taskOid,
                TaskOrErrorDetailOid: errorData.DetailOid,
                UserOid: userOid,
            });
            setErrorInProcess(!!result);
        } catch (error) {
            setErrorInProcess(false);
        } finally {
            setLoading(false);
    ...
    const addFavoriteTask = async (isActiveValue: boolean) => {
        ...
        try {
            ...
            await blsCore.services.errorService.addFavoriteError({
                ErrorOid: data.Oid,
                UserOid: userInfo.Oid,
    ...
    const canShowActionButtons = React.useMemo(() => {
        ...
        const sendUserOid = errorData?.SendUserOid ?? 0;       // atanan/iletilen
        const createdUserOid = errorData?.CreatedUserOid ?? 0; // ben açtıysam

        const isMine = userOid === sendUserOid || userOid === createdUserOid;

        return isMine;
    }, [selectedUserInfo, errorData]);
                    ...
                    {canShowActionButtons && (
                        <View style={styles.iconButtonContainer}>
                            <TouchableOpacity style={styles.iconButton} onPress={processTask}>
                                <Image source={errorInProcess ? stopIcon : startIcon} style={styles.icon} />
                            </TouchableOpacity>
                    ...
                    <FlatList
                            ...
                            <View>
                                ...
                                {/* Task Assignment Modal */}
                                {
                                    ...
                                    <TaskErrorAssignmentDialog
                                        ...
                                        checkIsTaskOrErrorInProcess={errorInProcess}
                                    />
                                }
                                ...
                                <View style={styles.fieldContainer}>
                                    <View style={styles.textAreaContainer2}>
                                        <Text style={styles.value}>{errorData?.ErrorTitle}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.combinedFieldContainer}>
                                    <View style={styles.row}>
                                        ...
                                        <Text style={styles.value}>{errorData?.ProjectName}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Kurum Adı:</Text>
                                        <Text style={styles.value}>{errorData?.CompanyName}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Hata No:</Text>
                                        <Text style={styles.value}>{errorData?.ErrorNo}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Atanan Kullanıcı:</Text>
                                        <Text style={styles.value}>{errorData?.AssignedUserName}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Modül:</Text>
                                        <Text style={styles.value}>{errorData?.ModuleName}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Tahmini Bitiş Tarihi:</Text>
                                        <Text style={styles.value}>{errorData?.EstimatedFinishDate}</Text>
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={styles.row}>
                                        <Text style={styles.label}>Önem Derecesi:</Text>
                                        <Text style={styles.value}>{errorData?.PriorityName}</Text>
                                    </View>
                                </View>
                                <View style={styles.fieldContainer}>
                        ...
                        ListFooterComponent={
                            <View style={styles.requestHistoryContainer}>
                                ...
                                {errorDetailList.length > 0 ? (
                                    errorDetailList.slice(0, showAllDetailList ? errorDetailList.length : 1).map((request) => (
