...
function ErrorUpdate() {
    ...
    const dtEstimateFinishDate = useImpDateInput({ label: 'Tahmini Bitiş Tarihi' });
    ...
    const getParameterList = async () => {
        try {
            ...
            slcAssignedUser.setOptionList(users)
    ...
    const saveError = async () => {
        try {
            ...
            obj.ErrorTitle = txtErrorTitle.value;
            obj.ErrorExplanation = txtRichEditor.value;
            ...
            obj.ErrorStatusCode = ParametrConstants.TASK_STATUS_PENDING;
            ...
            obj.ErrorDocumentList = documentList;
            obj.ErrorDetail = {
                Description: 'Yeni Hata Alındı',
                CreatedUserOid: userInfo.Oid,
                SendUserOid: slcAssignedUser.value.Oid,
                ErrorStatusCode: ParametrConstants.TASK_STATUS_PENDING,
                InProgress: '1',
                TaskOid: 0,
            } as any;
            const errorResponse = await blsCore.services.errorService.saveError(obj)
            if (errorResponse && errorResponse.Oid > 0) {
                Toast.show({ type: 'success', text1: `Talebiniz alınmıştır. Hata numaranız ${errorResponse?.TaskNo.toString()}` });
            }
            setLoading(false)
        }
        catch (e) {
    ...
    const fillPage = (data:any) =>{
        try{
            txtErrorTitle.setValue(data.ErrorTitle);
            txtRichEditor.setValue(data.ErrorExplanation);
            slcProject.setValue({Oid:data.ProjectOid,ProjectName:data.ProjectName});
            slcMainModule.select(data.ModuleOid);
            slcModule.select(data.ModuleOid);
            slcSubModule.select(data.ModuleOid);
            slcAssignedUser.select(data.SendUserOid);
            dtEstimateFinishDate.setValue(data.EstimateFinishDate);
            slcPriority.setValue({ParameterCode:data.Priority,ParameterName:data.PriorityName});
            setLoading(false)
        }
        catch(e){
            setLoading(false);
                        ...
                        <View style={styles.scrollView}>
                            ...
                            <View style={styles.row}>
                                <View style={styles.halfWidth}>
                                    <ImpDateInput inputState={dtEstimateFinishDate} />
                                </View>
                            ...
                            <View style={styles.buttonRow}>
                                <ImpButton type="update" onPress={saveError} buttonStyle={{ width: '30%' }} buttonState={btnSave} />
